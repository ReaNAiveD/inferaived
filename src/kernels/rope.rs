...
impl RopeInplaceWebgpu {
    ...
    /// Bake the per-buffer bindings into a [`RopeInplaceWebgpuRunner`]
    ...
    pub fn plan(
    ...
    ) -> RopeInplaceWebgpuRunner {
        debug_assert_eq!(q.rank, 3);
        debug_assert_eq!(k.rank, 3);
        debug_assert_eq!(q.shape[0], k.shape[0]);
        debug_assert_eq!(q.shape[1] as usize, self.num_q_heads);
        debug_assert_eq!(k.shape[1] as usize, self.num_k_heads);
        debug_assert_eq!(q.shape[2] as usize, self.head_dim);
        debug_assert_eq!(k.shape[2] as usize, self.head_dim);
        ...
        let uniform_buffer = device.create_buffer(&wgpu::BufferDescriptor {
            label: Some("rope_runner/uniform_buffer"),
        ...
        let bind_group = device.create_bind_group(&wgpu::BindGroupDescriptor {
            label: Some("rope_runner/bind_group"),
        ...
        let workgroup_count = (total_invocations + Self::WORKGROUP_SIZE - 1) / Self::WORKGROUP_SIZE;
        RopeInplaceWebgpuRunner {
...
pub struct RopeInplaceWebgpuRunner {
    pipeline: wgpu::ComputePipeline,
    bind_group: wgpu::BindGroup,
    workgroup_count: u32,
}

impl RopeInplaceWebgpuRunner {
...
mod tests {
    ...
    async fn test_rope() {
        ...
        let runner = gpu.plan(&device, &queue, q_view, k_view, &position_buffer);
