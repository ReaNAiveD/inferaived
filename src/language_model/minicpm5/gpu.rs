...
use crate::{
    ...
    layers::minicpm5_layer_stack::{MiniCPM5LayerStackRunner, MiniCPM5LayerStackSession},
    lm_head::LmHeadWebgpuRunner,
    log_tensor,
    sampling::{SampledToken, SamplingParams, StoppingCriteria},
};

use super::{MiniCPM5Config, MiniCPM5ModelCore};

/// GPU-backend model for MiniCPM5: shared mid-stack + GPU embed
/// lookup + GPU sampler kernel. Per-request configuration is
/// `SamplingParams`.
pub struct MiniCPM5GpuModel {
    pub core: MiniCPM5ModelCore,
...
impl MiniCPM5GpuModel {
    pub fn new<'data>(
        ...
        let core = MiniCPM5ModelCore::new(device, queue, tensors, config);
...
struct GpuModelRunner {
    ...
    stack_runner: MiniCPM5LayerStackRunner,
...
impl<'m> MiniCPM5GpuSession<'m> {
    ...
    async fn run_and_read_back_token(
        ...
        let mut encoder = device.create_command_encoder(&wgpu::CommandEncoderDescriptor {
            label: Some("minicpm5_gpu_session/step_encoder"),
        });
        {
            let mut cpass = encoder.begin_compute_pass(&wgpu::ComputePassDescriptor {
                label: Some("minicpm5_gpu_session/step_compute_pass"),
